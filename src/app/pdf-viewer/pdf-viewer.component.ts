...
export class PdfViewerComponent
  ...
  @ViewChild('annotationCardContent', { static: false })
  annotationCardContent: ElementRef
  ...
  public annotationCardOffset: number | null
  public invertTriangle: boolean
  public offsetLeft: number
  public showAnnotations: boolean
  public pdfLoading: boolean = true
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (
      'activeAnnotation' in changes &&
      changes.activeAnnotation.currentValue
    ) {
      this.calculateCardPosition(this.activeAnnotation)
    }
  }
    ...
    document
      ...
      ?.addEventListener('mouseup', (event: any) => {
        ...
        if (!this.activeAnnotation) {
          ...
          this.calculateCardPosition(position)
  ...
  private calculateCardPosition(position: Coordinates): void {
    ...
    const offsetBottom =
      this.pdfViewerContainer.nativeElement.scrollHeight - position.positionY

    if (offsetRight < 360) {
      const adjustedCardWidth = 340

      this.annotationCardOffset = adjustedCardWidth - offsetRight

      adjustedPosition = {
        positionY: position.positionY,
        positionX: position.positionX - adjustedCardWidth + offsetRight,
      }
    } else if (position.positionX < 70) {
      adjustedPosition = {
        positionY: position.positionY,
        positionX: position.positionX + 70,
      }

      this.annotationCardOffset = -70
    } else {
      adjustedPosition = {
        positionY: position.positionY,
        positionX: position.positionX,
  ...
  private setPageOffsetLeft(): void {
    this.offsetLeft = (document.querySelector(
      '.page'
    ) as HTMLElement).offsetLeft
  }
}
